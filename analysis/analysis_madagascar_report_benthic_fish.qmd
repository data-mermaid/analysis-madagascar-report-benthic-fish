...
This example recreates figures from a "Madagascar Report Card" comparing benthic cover and fish biomass across surveyed sites. The code in this example is an adaptation of R code originally developed by James Robinson. Here we adapt the code to directly access data from the MERMAID platform ([https://datamermaid.org](https://datamermaid.org/)) and visualize it as James did in his original code.
...
## Stacked bar plots - Fish biomass by trophic group
...
### Reshape the data so that functional groups are in rows
...
#### Recreate the stacked barplot with fish biomass by functional group
fishBiomassPlot <- ggplot(data = fishTBL,
  ...
  scale_y_continuous(expand = c(0, 0),
                     limits = c(0, max(fishTBL$TotalBiomass) + 350)) +
  theme_classic() +
  theme(legend.position = "right",
        plot.title=element_text(colour = 'black', size = 14,
                                hjust = 0.5, face = 'bold'),
        legend.position.inside = c(0.7, 0.28),
        legend.box.background = element_blank(),
        legend.key = element_rect(color = "black", linewidth = 0.25),
        legend.title=element_text(colour='black', face='bold'),
...
## Scatterplot - Fish biomass vs. benthic cover categories
...
## Scatterplot - Trophic group biomass vs. hard coral cover
